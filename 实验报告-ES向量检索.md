...
2. 使用 BERT/Embedding 模型对法律文书数据进行编码，并将向量写入 Elasticsearch。
...
### 3.1 数据结构
`documents.json` 为 JSON 数组，每条数据包含：
- `id`：文档唯一标识
- `HTML_txt`：法律文书 HTML 内容

### 3.2 为什么要做 HTML → 纯文本
Embedding 模型主要针对自然语言文本。HTML 标签会引入噪声，因此先用 HTML 解析工具提取正文文本，再进行编码。
...
> **截图 1：索引 mapping（证明 dense_vector 向量索引）**  
> （在此处粘贴/插入截图）
...
1. 读取 `HTML_txt`
2. HTML 清洗得到 `text`
3. 使用 embedding 模型将 `text` 编码为 768 维向量
4. 写入 Elasticsearch 索引 `law_docs` 的 `embedding` 字段
...
程序输出示例包含：
- `Query = ...`
- Top 5 文档的 `score`、`id`、文本预览 `preview`

> **截图 2：向量检索结果（证明检索有效）**  
> （在此处粘贴/插入 `python query.py` 的终端输出截图）
...
本实验成功完成以下内容：
1. 在 Elasticsearch 中创建包含 `dense_vector` 字段的向量索引 `law_docs`，并启用向量索引（index=true）。
2. 使用中文 BERT embedding 模型将法律文书文本编码为 768 维向量并写入 Elasticsearch。
3. 使用 kNN 向量检索对输入查询进行相似度搜索，返回相关法律文书结果，证明向量检索有效。
